...
<script setup lang="ts">
  ...
  const applyFilterConditions = (filterConditions: Record<string, any>) => {
    ...
    // 处理风险等级 risk_level
    if (filterConditions.risk_level) {
      newModel.risk_level = [filterConditions.risk_level];
    }

    // 处理风险状态 status
    if (filterConditions.status) {
      newModel.status = [filterConditions.status];
    }

    // 处理责任人 operator
    if (filterConditions.operator) {
      newModel.operator = [filterConditions.operator];
    }

    // 处理当前处理人 current_operator
    if (filterConditions.current_operator) {
      newModel.current_operator = [filterConditions.current_operator];
    }

    // 处理通知人 notice_users
    if (filterConditions.notice_users) {
      newModel.notice_users = [filterConditions.notice_users];
    }

    // 处理策略ID strategy_id（逗号分隔的ID字符串）
    if (filterConditions.strategy_id) {
      newModel.strategy_id = filterConditions.strategy_id.split(',').map((id: string) => id.trim());
    }

    // 处理标签 tags（逗号分隔的ID字符串）
    if (filterConditions.tags) {
      newModel.tags = filterConditions.tags.split(',').map((id: string) => id.trim());
    }

    // 处理风险标记 risk_label
    if (filterConditions.risk_label) {
      newModel.risk_label = [filterConditions.risk_label];
    }

    // 处理风险ID risk_id
    if (filterConditions.risk_id) {
      newModel.risk_id = filterConditions.risk_id;
    }

    // 处理事件详情关键词 event_content
    if (filterConditions.event_content) {
      newModel.event_content = filterConditions.event_content;
    }

    // 处理风险标题关键词 title
    if (filterConditions.title) {
      newModel.title = filterConditions.title;
    }

    // 处理是否已生成报告 has_report
    if (filterConditions.has_report !== undefined) {
      newModel.has_report = [String(filterConditions.has_report)];
    }

    searchModel.value = newModel;

    // 处理事件字段筛选 event_filters
    if (filterConditions.event_filters && Array.isArray(filterConditions.event_filters)) {
    ...
    // 处理排序 sort（传入 searchModel 或通过 getSearchParams 透传给父组件）
    if (filterConditions.sort && Array.isArray(filterConditions.sort)) {
